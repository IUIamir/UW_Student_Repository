...
if Illuminant_index == 1
    title('Color chart under illuminant D65', 'FontSize', 20)
else
...
% Color chart under given illuminant
nexttile
imshow(mcc)
if Illuminant_index == 1
    title('Color chart under illuminant D65', 'FontSize', 15)
else
    title('Color chart under illuminant A', 'FontSize', 15)
end

% White balanced image of the color chart
nexttile
imshow(mcc_wb)
title('White balanced image of the color chart', 'FontSize', 15)
...
save_path = fullfile(Path_to_your_repository, '\Lab1');
saveas(gcf, fullfile(save_path, 'Macbeth_simulation.png'));
